...
class Preprocessor(object):
	def __init__(self):
		self.emp = re.compile(u'\s')
		pass

	def washEmpty(self, s):
		return self.emp.sub(u'', s)
...
class QTSPreprocessor(Preprocessor):
	def __init__(self):
		...
		self.poemMa = re.compile(u'^.*?〖(.*?)〗.*?作者:(.*?)\n(.*?)$', re.S)
		self.poemSub = re.compile(u'\(.*?\)|（.*?）')
	...
	def readData(self, filepath):
		...
		"""
		data = open(filepath).read().decode('utf-8')    #unicode
		rawList = re.split(u'<http.*?>', data)
		poemSet = set()
	...
	def savePair(self, pairList, filepath):
		with open(filepath, 'w') as f:
			for i in range(len(pairList)-1):
				FS, SS = pairList[i]
				f.write('%s %s\n' % (FS.encode('utf-8'), SS.encode('utf-8')))
			FS, SS = pairList[-1]
			f.write('%s %s' % (FS.encode('utf-8'), SS.encode('utf-8')))
	...
	def saveSen(self, senList, filepath):
		with open(filepath, 'w') as f:
			for i in range(len(senList) - 1):
				f.write((senList[i] + u'\n').encode('utf-8'))
			f.write(senList[-1].encode('utf-8'))
...
if __name__ == '__main__':
	...
	poemList = proprocessor.readData(DATA_PATH + '/作诗/全唐诗.txt')
	print 'poem number=%d,' % (len(poemList)), printStatistic([re.sub(u'。|，', u'', poem) for poem in poemList])
