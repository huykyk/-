...
class SMTModel(object):
	...
	def calTransDict(self):
		print 'Traning Translation Model...'
	...
	def generateSS(self, FS):
		...
		for fw in FS:
			...
			# 	print beam[i]['logp'], u''.join(beam[i]['SS']), u';',
			# print ''
			logpList = []   # [(logp, index(in beam), word), ...]
			trlogpList = np.array(map(lambda sw: self.calTranslogp(sw, fw), self.trainVList))   # shape=(transV,)
			for j in range(len(beam)):
				given = beam[j]['SS'][len(beam[j]['SS'])-self.LM_GRAM_NUM+1:]
				lmlogpList = np.array(map(lambda sw: self.calLMlogp(given, sw), self.trainVList))
				logpJ = (1.0 - self.LM_WEIGHT)*trlogpList + self.LM_WEIGHT*lmlogpList + beam[j]['logp']
				logpList.extend(map(lambda logp, sw: (logp, j, sw), logpJ, self.trainVList))
			...
			beam = newBeam
		for i in range(len(beam)):
			beam[i]['SS'] = u''.join(self.stripNoChar(beam[i]['SS']))
		return beam
	...
	def generateSSList(self, FSList, pFlag=False):
		...
		for i in range(size):
			...
			if pFlag and i % 20 == 0:
				print '%.2f%%(%d/%d) SS generated' % (100.0*(i+1)/size, i+1, size)
		return TSSList
...
if __name__ == '__main__':
	...
	parser = OptionParser()
	parser.add_option("-m", "--mode",  default = INTERACTIVE_MODE, help = "running mode: %s|%s; DEFAULT=%s" % (INTERACTIVE_MODE, TESTFILE_MODE, INTERACTIVE_MODE), action = "store", type = "string", dest = "mode")
